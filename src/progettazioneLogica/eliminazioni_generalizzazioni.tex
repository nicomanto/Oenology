...
\begin{flushleft}
	...
	La seconda relazione si chiama \textbf{IsPrivato} ed è stata creata utilizzando lo stesso ragionamento di IsAzienda, ovvero la cardinalità da \emph{Acquirente} a \emph{Privato} è $(0,1)$ poichè non tutti gli acquirenti sono privati, mentre da \emph{Privato} ad \emph{Acquirente} è $(1,1)$ poichè un privato è sempre un acquirente. Le entità \emph{Azienda} e \emph{Privato} mantengono gli attributi visti in precedenza, in più ad \emph{Acquirente} verrà aggiunto l'attributo \emph{NumAcquirente} che sarà chiave referenziale ad \emph{Acquirente}. Si è deciso di aggiungere questo attributo consapevoli dell'eventualità che quest'ultimo possa essere \textbf{NULL} visto che il numero di aziende che non sono acquirenti (es. fornitori) è minore di quelle che lo sono.
\end{flushleft}
